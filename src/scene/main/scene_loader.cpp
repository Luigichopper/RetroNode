    ...
    if (j.contains("instance")) {
        ...
        if (!final_path.empty()) {
            ...
            if (instanced_node) {
                ...
                std::cout << "[SceneLoader] Instanced sub-scene: " << instance_path << " -> " << final_path << std::endl;
...
Node* SceneLoader::load_scene_from_binary(const std::string& filepath) {
    ...
    if (!file.is_open()) {
        std::cerr << "[SceneLoader] Failed to open binary scene file: " << filepath << std::endl;
        return nullptr;
    }
    ...
    if (std::string(magic, 4) != "RNB1") {
        std::cerr << "[SceneLoader] Invalid binary map signature in " << filepath << std::endl;
        return nullptr;
    }

    Node* root = parse_node_binary_internal(file);
    if (root) {
        std::cout << "[SceneLoader] Loaded binary scene file (.rnb): " << filepath << std::endl;
    }
    return root;
}

Node* SceneLoader::load_scene_from_file(const std::string& filepath) {
    if (filepath.length() > 4 && filepath.substr(filepath.length() - 4) == ".rnb") {
